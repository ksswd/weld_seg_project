...
def visualize_colored_point_cloud(point_cloud, labels, method="open3d", output_path=None):
    """
    根据标签可视化彩色点云。
    ...
    if method == "open3d":
        ...
        plt.savefig("colored_point_cloud.png" if output_path is None else output_path)
        plt.show()
    else:
...
if __name__ == "__main__":
    # 1. 替换为你的.npz文件路径
    npz_file_path = "./data/predictions/new/MeshE1_seg_label_pred.npz"  # 示例："data/labeled_pcd.npz"
    print(f"正在加载点云文件：{os.path.abspath(npz_file_path)}")
    # 2. 读取点云和标签
    # 注意：这里的键名是 "features"
